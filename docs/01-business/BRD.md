...
personalized value
- Carries a short-term savings goal — a car, a trip, or a new laptop — 
with no structured plan or timeline to reach it
...
1. Set a savings goal with a defined purpose, target amount,
  and deadline
2. Log income and daily expenses in under 60 seconds
...
4. Request AI-generated advices on a weekly basis that reads their
  actual spending data and produces a specific, personalized 
   plan — including which categories to cut, by how much, and 
   which alternative substitutes (such as grocery retailers and 
   specific food ingredients) to consider

The weekly AI advisory layer is powered by the Google Gemini API 
via Google AI Studio. The app passes the user's entered income, 
spending by category, and savings goal directly into a structured prompt. 
Gemini returns specific, contextual advice — not generic financial tips 
...
- Single permanent savings goal setup: user selects a goal type, enters a target  amount, and sets a deadline
- Visual savings progress indicator and spending behavior 
visualizations
- Weekly AI advice report generation powered by the Google Gemini API (from Google AI Studio)
...
- Multiple savings goals creation, prioritization, and tracking
- AI-generated savings goal creation — a mode where the 
...
- The Google Gemini API remains available and within
low-cost usage limits for a single-user application
- Supabase free tier is sufficient for the storage and query volume generated by a small user base
...
| **Budget**         | $20 total project cap. All cloud services and design tools operate strictly on free tiers (Supabase, Vercel, Figma, GitHub, and Google AI Studio). The sole expense was a single-month Cursor Pro subscription ($20) to leverage AI coding agent features for rapid development. |
...
| ----------------------------------------- | ---------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Feature creep extends timeline            | Medium     | High   | Strict prioritization enforced through the simplified FRD; all out-of-scope items deferred                                                                                                                                            |
| Supabase free tier limitations            | Low        | Medium | Data volume remains minimal — the app targets a small user base with lightweight transaction records; Supabase free tier supports up to 50,000 monthly active users and 500MB database storage, both well beyond this project's needs |
